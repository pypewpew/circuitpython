...
def tick(delay):
    ...
    _tick = (_tick + int(delay * 0x20b0)) % 0xffff
...
def keys():
    ...
    for button in _buttons:
        mask = (mask << 1) | button.value
    return mask


def init():
    global _screen, _tick, keys, _rows, _cols, _buttons
    ...
    _buttons = (
            touchio.TouchIn(board._U),
            touchio.TouchIn(board._D),
            touchio.TouchIn(board._L),
            touchio.TouchIn(board._R),
            touchio.TouchIn(board._O),
            touchio.TouchIn(board._X),
    )
    _pew.PewPew(_screen.buffer, _rows, _cols)
